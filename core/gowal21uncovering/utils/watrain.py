...
from core.utils.metrics import accuracy

from core.utils.context import ctx_noparamgrad_and_eval
from core.utils.train import Trainer
...
class WATrainer(Trainer):
    ...
    """
    def __init__(self, info, args, num_samples):
        ...
        if info['data'] in ['cifar10', 'cifar10_augmented1m']:
            self.num_classes = 10
        elif self.params.dataset in ['cifar100', 'cifar100s']:
            self.num_classes = 100
        elif self.params.dataset == 'tiny-imagenet':
            self.num_classes = 200
        self.update_steps = int(np.floor(num_samples/self.params.batch_size) + 1)
        self.warmup_steps = 0.025 * self.params.epochs * self.update_steps
    ...
    def autoattack_loss(self, data_loader, logger_path):
        norm = 'Linf' if self.params.attack in ['fgsm', 'linf-pgd', 'linf-df'] else 'L2'
        self.wa_model.eval()
        adversary = AutoAttack(self.wa_model, norm=norm, eps=self.params.attack_eps,
                               version='standard', log_path=logger_path)

        l = [x for (x, y) in data_loader]
        x_test = torch.cat(l, 0)
        l = [y for (x, y) in data_loader]
    ...
    def eval(self, dataloader, adversarial=False):
        ...
        for x, y in dataloader:
            ...
            if adversarial:
                ...
                out = self.wa_model(x)
            acc += torch.sum(y.eq(out.max(dim=1)[1]))
    ...
    def save_model_resume(self, path, epoch, run_id, best_acc, record_predictions, record_predictions_adversarial):
        """
        ...
        torch.save({
            ...
            "record_predictions_adversarial": record_predictions_adversarial
        }, path)
    ...
    def load_model_resume(self, path):
        ...
        # Catch cases where best_acc was not saved in checkpoint dict
        try:
            best_acc = checkpoint['best_acc']
        except KeyError:
            best_acc = np.zeros(1)
        try:
            record_predictions = checkpoint['record_predictions']
        except KeyError:
            record_predictions = False
        try:
            record_predictions_adversarial = checkpoint['record_predictions_adversarial']
        except KeyError:
            record_predictions_adversarial = False

        return checkpoint['epoch'], checkpoint['run_id'], best_acc, record_predictions, record_predictions_adversarial
