...
class Trainer(object):
    ...
    def predictions(self, dataloader, adversarial=False):
        self.model.eval()
        ...
        for x, y in dataloader:
            ...
            if adversarial:
                ...
                out = self.model(x)
            y_predictions.append(torch.softmax(out, dim=1).gather(1, y.unsqueeze(1)).cpu().detach().numpy())
        return np.concatenate(y_predictions).squeeze()
